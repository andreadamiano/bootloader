...
class SerialRepository:
    ...
    def send_firmware_update(self):
        ...
        with open("build/main_app.bin", "rb") as f:
            ...
            #send firware size
            frame = create_frame(SupId.DATA, data_lenght.to_bytes(2, "little"))  # send firmware size as 2 bytes in little indian format 
            self.send_frame_and_wait_ack(frame)
            ...
            for i in range(0, data_lenght, MAX_PAYLOAD_SIZE):
                ...
                if not self.send_frame_and_wait_ack(frame):
                    logger.critical(f"An error occurred while uploading the firmware exiting...")
                    return
